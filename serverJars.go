...
func getJarfileVanilla(w http.ResponseWriter, r *http.Request) {
	...
	if version != "" {
		...
		fmt.Fprintf(w, "{\"version\":\"%v\",\"platform\":\"vanilla\",\"url\":\"%v\"}", version, jarfile[version])
	} else {
		fmt.Fprintf(w, "{\"err\": \"No version input\"}")
	}
}

func getJarfileForge(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("content-type", "application/json; charset=utf-8")
	version := r.URL.Query().Get("v")
	...
	if version != "" {
		...
		fmt.Fprintf(w, "{\"version\":\"%v\",\"platform\":\"forge\",\"url\":\"%v\"}", version, jarfile[version])
	} else {
		fmt.Fprintf(w, "{\"err\": \"No version input\"}")
	}
...
func getJarfileFabric(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("content-type", "application/json; charset=utf-8")
	version := r.URL.Query().Get("v")

	invalidFabric := []string{"1.12", "1.12.1", "1.12.2", "1.13", "1.13.1", "1.13.2"}

	// Make sure the version is fabric supported
	if slices.Contains(invalidFabric, version) {
		fmt.Fprintf(w, "{\"err\": \"Fabric only supports 1.14+\"}")
	} else if version != "" {
		// Create map to store the URLs of each server type jarfile
		jarfile := make(map[string]string)
		...
		fmt.Fprintf(w, "{\"version\":\"%v\",\"platform\":\"fabric\",\"url\":\"%v\"}", version, jarfile[version])
	} else {
		fmt.Fprintf(w, "{\"err\": \"No version input\"}")
	}
}
