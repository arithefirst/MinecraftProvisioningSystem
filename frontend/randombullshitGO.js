...
function sendRequest() {
    // Put all the values of the toggles in the "bool" map
    let bools;
    bools = new Map();
    ...
    // Put all the strings in the "strings" map
    ...
    // Put all the ints in the "ints" map
    const ints = new Map();
    ...
    // Iterate through all the ints
    for (let [key, value] of ints) {
        // "key": value,
        jsonString = jsonString + '"' + key + '"' + ": " + value + ", ";
    }

    // Remove the last ", " and close the json with a curly brace
...
function getData() {
    ...
    let json = encodeURI(getUrlVars()["jsonString"]);
    console.log(json);
    const url = `${baseUrl}/api/v1/server-properties?jsonString=${json}`;
    http.open("GET", url);
    http.send();

    http.onreadystatechange = () => pageSet(http.responseText);
}
